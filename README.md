# channels-demo plugin

This project contains a simple Nextflow plugin called `channels-demo` which provides examples of different plugin extensions:
...
NOTE: If you want to use this project as a starting point for a custom plugin, you must rename the `plugins/channels-demo` folder and update `settings.gradle` with your plugin name.
...
- `plugins/channels-demo`

    The plugin implementation base directory.

- `plugins/channels-demo/build.gradle`

    Plugin Gradle build file. Project dependencies should be added here.

- `plugins/channels-demo/src/resources/META-INF/MANIFEST.MF`
...
- `plugins/channels-demo/src/resources/META-INF/extensions.idx`
...
- `plugins/channels-demo/src/main`

    The plugin implementation sources.

- `plugins/channels-demo/src/test`
...
4. Run Nextflow with the plugin, using `./launch.sh` as a drop-in replacement for the `nextflow` command, and adding the option `-plugins channels-demo` to load the plugin:
    ```bash
    ./launch.sh run nextflow-io/hello -plugins channels-demo
    ```
...
The project should be hosted in a GitHub repository whose name matches the name of the plugin, that is the name of the directory in the `plugins` folder (e.g. `channels-demo`).
...
2. Use the following command to package and create a release for your plugin on GitHub:
    ```bash
    ./gradlew :plugins:channels-demo:upload
    ```
